...
public class Model {
	...
	/**
	 * Checks user inputs as to whether they want to add or clear assemblies
	 */
	...
	/**
	 * Drags the mass closest to the mouse click and deletes the MouseDragger
	 * the mouse is not currents being pressed
	 */
	private void dragMass(double elapsedTime, Dimension bounds) {
		if (myView.getLastMousePosition() != null) {
			if (myMouseDragger == null)
				myMouseDragger = new MouseDragger(myView, myAssemblies);
			else {
